...
for row in sys.stdin:
  ...
  patient_id, sentence_id, start_position, mention_id, lemmas_phrase = row.strip().split('t')
  sentence_ids = sentence_id.split(ARR_DELIM);
  start_positions = start_position.split(ARR_DELIM);
  ...
  for sp_token, sp_weight, sp_mention_id in featureBins[0]:
    ...
    for grade in xrange(1,5):
      for token, weight, mention_id in featureBins[grade]:
        ...
        tallyBins[grade] += min(0, sp_weight - abs(sp_sent_id - sent_id))
  ...
  if ( tallyBins[sortedIndex[1]] < BETWEEN_RATIO * tallyBins[sortedIndex[0]] and 
       abs(sortedIndex[1] - sortedIndex[0]) <= 1 ):
    # transition between two grades
    predicted_grade = str((sortedIndex[0]+sortedIndex[1])/2)
